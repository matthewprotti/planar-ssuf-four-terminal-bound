...
Single-source unsplittable-flow rounding converts a fractional flow into one source-terminal path per demand. The classical cost-constrained question asks for a routing that does not exceed the fractional cost while controlling arc overload. The present note keeps one compact four-terminal planar DAG fixed and imposes two simultaneous linear cost budgets. It asks for the exact worst-case normalized additive upper arc deviation on that gadget.

The model is intentionally narrow. A routing is feasible when it satisfies scenario-wise cost non-increase in both scenarios; equality is not required. The theorem does not determine the unrestricted planar constant, the otherwise-identical arbitrary one-scenario fixed-graph constant, or a many-scenario constant.

Dmitry Rybin's public counterexample of 22 July 2026 was the direct catalyst for this research line. The earlier release in this repository established a one-scenario planar lower bound and restricted equal-cost sharpness result. The new theorem is a separate fixed-gadget two-scenario result.

# The graph, paths, and flow model
...
![Released four-terminal gadget](paper/rb003_four_terminal_gadget.pdf){width=88%}
...
# Scenario-wise cost non-increase
...
# E-side normalization and graph identities
...
# Upper bound: reduction to a star-triangle
...
## All four terminals individually omittable
...
## Exactly three terminals individually omittable
...
# Exact star-triangle optimization

## The non-omittable terminal is central: \(u\in\{2,3\}\)
...
## The non-omittable terminal is outer: \(u\in\{1,4\}\)
...
The foundational single-source unsplittable-flow framework is due to Dinitz, Garg, and Goemans. Traub, Vargas Koch, and Zenklusen prove a planar cost-preserving upper bound with one cost vector, while Majthoub Almoghrabi, Skutella, and Warode obtain strong series-parallel integrality and unsplittable-flow results. Morell and Skutella study simultaneous arc-wise lower and upper load bounds. Multicriteria, robust, and reroutable flow models supply important adjacent context, but use different constraints or uncertainty semantics.

The four-label threshold-family calculations used in the finite registry overlap classical weighted-simple-game and threshold-function theory; no novelty claim is made for that general classification. A targeted search on 28 July 2026 found no indexed match for the exact fixed-gadget, shared-baseline two-scenario formulation, the star-triangle proof, or the value 17/8. This is a limited search result, not exhaustive novelty clearance.
...
OpenAI's 28 July 2026 exploratory field report on agentic scientific computing describes a broad shift from implementation toward human specification, validation, orchestration, and stewardship. It is cited here as contemporaneous methodological context, not as validation or endorsement of this result.
...
1. Y. Dinitz, N. Garg, and M. X. Goemans, "On the single-source unsplittable flow problem," *Combinatorica* 19 (1999), 17-41. DOI: 10.1007/s004930050043.
2. S. Morell and M. Skutella, "Single source unsplittable flows with arc-wise lower and upper bounds," *Mathematical Programming* 192 (2022), 477-496. DOI: 10.1007/s10107-021-01704-4.
3. V. Traub, L. Vargas Koch, and R. Zenklusen, "Single-source unsplittable flows in planar and bounded-genus graphs," *Mathematical Programming* (2026). DOI: 10.1007/s10107-026-02365-x.
4. M. Majthoub Almoghrabi, M. Skutella, and P. Warode, "Integer and unsplittable multiflows in series-parallel digraphs," *Mathematical Programming* (2026). DOI: 10.1007/s10107-026-02392-8.
5. OpenAI et al., "Scientific computing in the age of agentic AI: an exploratory field report," 28 July 2026, https://cdn.openai.com/pdf/scientific-computing-in-the-age-of-agentic-ai-an-exploratory-field-report.pdf.
