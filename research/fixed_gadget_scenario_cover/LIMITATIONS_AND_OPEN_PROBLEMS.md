...
- Positive multi-scenario results do not extend to signed or zero-coordinate
  scenarios.
- GM-002 gives a supremum and makes no attainment or nonattainment claim.
- The exact two-scenario bounded-heterogeneity curve remains open below
  `kappa_0`; the upper bound by two is not an equality classification.
- SC-006 is a fixed-family phase theorem, not the global bounded-`kappa` curve.
- The F064 value `5/2 - sqrt(2)` has a checked strict lower sequence but no
  matching cell-specific upper proof and remains unclaimed.
- Fixed-instance atlas values and cover counts do not imply global upper
  bounds.
- Exact checkers do not certify unencoded orientation, compactness, case
