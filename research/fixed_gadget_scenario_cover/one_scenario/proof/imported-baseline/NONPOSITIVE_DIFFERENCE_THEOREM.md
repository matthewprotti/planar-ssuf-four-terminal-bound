...
The switch preserves feasibility, weakly decreases every trunk deviation, and
leaves every positive private-arc deviation at most one. It need not weakly
improve the complete route value because a private deviation can increase.
Nevertheless, for every target upper bound at least one, it is sufficient to
seek a feasible candidate with all nonpositive coordinates routed E and then
bound only its trunk deviation.
...
## 4. Chain type: upper envelope \(9/8\)
...
\[
U_{AC}=\max\{e_A,e_C,e_A+e_C-\ell_B\}.
\]

These quantities are conservative trunk envelopes: when the omitted E-routed
terminal has a nonzero negative trunk contribution, the actual trunk maximum
can be smaller. The complete route values are bounded respectively by
\(\max\{1,U_{AB}\}\), \(\max\{1,U_{BC}\}\), and
\(\max\{1,U_{AC}\}\). Only this upper-bound direction is used below.
...
Put `s=e_A+e_C`. The smaller of the first two envelopes is at most

\[
e_B+\frac{s}{2}.
\]

If `U_AC<=1`, there is nothing to prove. Otherwise its term above one must be
